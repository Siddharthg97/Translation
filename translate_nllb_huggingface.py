...
def text_translation(text1,model=1,lang_code = 'en'):
    # print(result.values)
...
def text_preproc(text1):
    ...
    for i in range(len(text1_split)):
        ...
        if i<len(text1_split)-1 and len(current_text) + len(text1_split[i+1])<length_max:
            ...
            text_new_temp = text_translation(current_text,1,lang_code)
...
if __name__ == "__main__":
    ...
    for i in range(df_to_be_translated.shape[0]):
        ...
        # print(text3)
        df_to_be_translated["Translated_Text"].iloc[i] = text3
