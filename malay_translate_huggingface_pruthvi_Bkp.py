...
def text_preproc(text1):
    ...
    for i in range(len(text1_split)):
        # print(len(text1_split),'lenght',i)
        # for i in range(len(text1_split)):
        
        split_list = []
        result = detect(text=text1_split[i], low_memory=False)
        # print(result)
        ...
        if tag_is_valid(text_lang):
            ...
            print(lang_name)
            if lang_name!= lang_name_current or i==0:
                for j in range(0,lang_model.shape[0]):
                    # print(lang_name,lang_model['V_LANGUAGE_NAME'].iloc[i])
                    if lang_name==lang_model['V_LANGUAGE_NAME'].iloc[j]:
                        # print(lang_name,lang_model['V_LANGUAGE_NAME'].iloc[i])
                        model_code = int(lang_model['N_MODEL_CODE'].iloc[j])
                        model_name = lang_model['V_MODEL_NAME'].iloc[j]
                        break
                    else:
                        # print('else')
                        model_code = 1
